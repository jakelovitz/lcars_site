...
class LibraryPage extends Component {
  ...
  fetchBooks = () => {
    fetchAndParseBookList()
      .then(books => {
        ...
        this.setState({
          books,
          filteredBooks: books, // Initially, no filter is applied
          genres,
          loading: false
        });
      })
  ...
  handleGenreChange = (event) => {
    this.setState({ currentGenre: event.target.value }, this.filterBooksByGenre);
  };
  ...
  renderDropdown = () => {
    ...
    return (
      <select onChange={this.handleGenreChange} value={this.state.currentGenre}>
        {genres.map(genre => (
  ...
  render() {
    const { loading, error, filteredBooks } = this.state;
