...
const mockBooks = [
    { title: 'Book 1', author: 'Author A', genre: 'Fiction', subGenre: 'Adventure', olid: 'OL12345M' },
    { title: 'Book 2', author: 'Author B', genre: 'Non-Fiction', subGenre: 'Biography', olid: 'OL67890M' },
];

describe('LibraryPage Component', () => {
    beforeEach(() => {
        fetchAndParseBookList.mockResolvedValue(mockBooks);
        sortBooks.mockImplementation((books) => books.sort((a, b) => a.author.localeCompare(b.author))); // Mock implementation for sorting
    });
    ...
    test('fetches and displays books on mount', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('book-OL12345M')).toBeInTheDocument());
        await waitFor(() => expect(screen.getByTestId('book-OL67890M')).toBeInTheDocument());
    });
    ...
    test('displays error message if fetching books fails', async () => {
        ...
        await waitFor(() => expect(screen.getByText('Error loading books: Failed to fetch books')).toBeInTheDocument());
    });
    ...
    test('renders genre and sub-genre dropdowns', async () => {
        ...
        await waitFor(() => expect(screen.getByText('Select a sub-genre...')).toBeInTheDocument());
    });
    ...
    test('filters books by genre and sub-genre', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('book-OL12345M')).toBeInTheDocument());
        await waitFor(() => expect(screen.queryByTestId('book-OL67890M')).not.toBeInTheDocument());
    ...
    test('sorts books by author and title', async () => {
        ...
        await waitFor(() => expect(sortBooks).toHaveBeenCalledWith(expect.any(Array)));
        await waitFor(() => expect(screen.getByText('Sorted by: Author')).toBeInTheDocument());

        await act(async () => {
            fireEvent.click(screen.getByText('Sort by Title'));
        });

        await waitFor(() => expect(screen.getByText('Sorted by: Title')).toBeInTheDocument());
    });
});
